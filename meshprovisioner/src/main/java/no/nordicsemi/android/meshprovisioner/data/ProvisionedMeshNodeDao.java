...
public interface ProvisionedMeshNodeDao {
    ...
    @Query("SELECT * from nodes WHERE mesh_uuid IS :meshUuid")
    LiveData<List<ProvisionedMeshNode>> getNodesLiveData(final String meshUuid);
    ...
    @Query("SELECT * from nodes WHERE uuid IS :nodeUUID")
    ProvisionedMeshNode getNode(final String nodeUUID);
}
